    ...
    if password is None:
        ...
        if os.path.exists(fich):
            ...
            if summary == True:
                ...
                set_level(int(level))                 
            compressed_file = open(fich.rsplit('.', 1)[0]+"."+DEFAULT_EXT, "ab") #opens in append mode to write data
            encode(text_File.read()) # Runs encode cicle
            print("The compressed file", fich.rsplit('.', 1)[0]+"."+DEFAULT_EXT, "has been sucessfuly created !\n")
            text_File.close()
            compressed_file.close()
        else:
...
def decompress_file(fich, level, password, summary):
    ...
    if password is None:
        ...
        if os.path.exists(fich):
            split_ex = os.path.splitext(fich)
            if split_ex[1].lower() != ".LZS":
                print(f'The specified {fich} file is not a .lzs compressed file.\nExiting...')
                sys.exit(2)
